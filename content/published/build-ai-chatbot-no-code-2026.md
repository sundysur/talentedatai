...
Now imagine a chatbot that answers instantly, in plain English, with exactly the right information. It handles follow-ups, stays on topic, and asks for their email so you can follow up in the morning. It runs 24 hours a day without you doing anything.
...
- A free **API key** from Anthropic (for Claude) or OpenAI (for GPT-4), covered in Step 2
- A browser and around 90 minutes
- A clear idea of what you want your chatbot to do and what it should know

That's it. You don't need to download anything. You don't need to set up a coding environment. You don't need a paid plan to get started. Everything in this guide works on free tiers.
...
**Voiceflow** is the better choice for more complex conversational flows: branching paths, multi-step logic, voice interfaces. More capable, but a steeper learning curve and a more limited free tier.
...
The intelligence comes from a large language model: Claude (made by Anthropic) or GPT-4 (made by OpenAI). These are the same AI models behind Claude.ai and ChatGPT. To connect them to your chatbot, you need an **API key**, a password that lets Botpress talk to the AI on your behalf.
...
In Botpress, paste your system prompt into the **Agent Instructions** field in the bot editor. Botpress applies this to every conversation.
...
- **It sounds robotic or stiff.** Add tone guidance to your system prompt: "Use a warm, conversational tone. Keep responses concise, two to three sentences unless a longer answer is clearly needed."
...
If you use **WordPress**: go to Appearance → Theme Editor → footer.php and paste the snippet above the `</body>` tag. Alternatively, install the free plugin **Insert Headers and Footers**, which lets you paste the snippet without touching theme files.
